...
def play_turn(board, sign, selected_pos):
    if position_available(board, selected_pos):
        ...
        current_selected_position = input(f'select a valid available position [0-9]: ')
...
def is_row_winner():
    for row_number in range(len(setup.board)):
        if setup.board[row_number].count("X") == 3 or setup.board[row_number].count("O") == 3 != setup.empty_spots:
            return True
    return False
...
def is_diagonal_winner():
    left_d = setup.board[0][0] == setup.board[1][1] == setup.board[2][2] != setup.empty_spots
    rigth_d = setup.board[0][2] == setup.board[1][1] == setup.board[2][0] != setup.empty_spots
    if left_d or rigth_d:
        return True
    return False
...
def check_selected_position_is_valid(pos, counter):
    if not pos.isdigit() or not is_position_valid(pos):
        pos = input(f'{setup.player_names[counter % 2]}, select a position number between 0-9: ')
        return check_selected_position_is_valid(pos, counter)
    return pos
...
def play():
    ...
    if not has_moves(turns_counter) and not has_won():
        ...
        return
    turns_counter -= 1
    print(f"{setup.player_names[turns_counter % 2]} has won !")
